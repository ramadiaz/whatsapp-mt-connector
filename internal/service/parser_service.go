...
func (s *ParserService) ParseText(ctx context.Context, userUUID string, text, quotedBody string, activePendings []*transaction.PendingTransactionRow, mtClient moneytracker.MoneyTrackerClient) (*ninerouter.AIExtractionResult, error) {
	...
	pendingContext := ""
	if len(activePendings) > 0 {
		var sb strings.Builder
		for i, pt := range activePendings {
			sb.WriteString(fmt.Sprintf("- Transaction %d: type=%s, amount=%s, category_hint=%s, account_hint=%s, date=%s, remark=%s\n",
				i+1, pt.Type, pt.Amount, pt.CategoryHint, pt.AccountHint, pt.TransactionDate, pt.Remark))
		}
		pendingContext = sb.String()
	}

	prompt := ninerouter.BuildTextPrompt(text, quotedBody, today, catLabels, accLabels, userContext, pendingContext)
...
func (s *ParserService) ParseImage(ctx context.Context, userUUID string, messageID, phone, caption, quotedBody string, activePendings []*transaction.PendingTransactionRow, mtClient moneytracker.MoneyTrackerClient) (*ninerouter.AIExtractionResult, error) {
	...
	prompt := ninerouter.BuildImagePrompt(caption, quotedBody, today, catLabels, accLabels, userContext, pendingContext)
